...
export interface CleaningJob {
  ...
  timeWindow: string;    // e.g. "10:00 - 14:00"
...
export interface LostItem {
  ...
  guestName?: string;
  bookingId?: string;
  storageLocation: string; // e.g., "Locker B-3"
  photoUrl?: string;
...
export interface ExtraService {
  ...
  apartmentName: string;
  guestName: string;
  serviceName: string; // e.g. "Airport transfer", "Late checkout", "Baby cot"
  price: number;
  date: string;
...
export interface ActivityLog {
  ...
  details: string;
  timestamp: string;
  entityType: 'booking' | 'cleaning' | 'maintenance' | 'linen' | 'lost_item' | 'system';
  entityId?: string;
}
